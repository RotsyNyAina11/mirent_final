  ...
  const exportSingleProformaToPDF = async (
    ...
    autoTable(doc, {
      ...
      didDrawPage: (data) => {
        ...
        // ✅ Date et Lieu en bas du tableau
        const generationDate = new Date();
        const formattedGenerationDate = `${generationDate.getDate()} Avril ${generationDate.getFullYear()}`;
        const locationText = `Antananarivo le ${formattedGenerationDate}`;
        const finalYLocation = finalYDateCreation + 6;
        const tableRightMargin = 20;

        // Ajustez l'espacement vertical pour le lieu

        doc.setFontSize(12);
        doc.text(locationText, pageWidth - tableRightMargin, finalYLocation, {
          align: "right",
        });
        ...
        {proformas.length > 0 && (
          <DashboardCard sx={{ marginTop: 2 }}>
            <Stack
              ...
              sx={{ padding: 2 }}
            >
              <Typography color="green">Liste des Proformas</Typography>
            </Stack>
