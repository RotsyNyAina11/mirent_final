...
import { createProforma, fetchAvailableVehicles } from "../../../redux/features/proforma/proformaSlice";
...
const CreateProforma: React.FC = () => {
  ...
  const { availableVehicles, loading, error } = useSelector((state: RootState) => state.proformas);
  ...
  useEffect(() => {
    const fetchClients = async () => {
      try {
        ...
        setSnackbarMessage(error instanceof Error ? error.message : "Erreur lors de la récupération des clients");
  ...
  useEffect(() => {
    const fetchAllVehicles = async () => {
      try {
        ...
        setSnackbarMessage(error instanceof Error ? error.message : "Erreur lors de la récupération des véhicules");
        setSnackbarSeverity("error");
        setOpenSnackbar(true);
      }
    };
  ...
  useEffect(() => {
    ...
    const startDateValid = formData.startDate && dateRegex.test(formData.startDate);
    ...
    if (startDateValid && endDateValid) {
      ...
      if (!isNaN(startDate.getTime()) && !isNaN(endDate.getTime()) && endDate > startDate) {
        dispatch(fetchAvailableVehicles({ dateDepart: formData.startDate, dateRetour: formData.endDate }));
      }
    }
  }, [formData.startDate, formData.endDate, dispatch]);

  const vehiclesToDisplay = formData.startDate && formData.endDate ? availableVehicles : allVehicles;
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.vehicleId) errors.vehicleId = "Veuillez sélectionner un véhicule";
    if (!formData.startDate) errors.startDate = "Veuillez entrer une date de début";
    else if (!/^\d{4}-\d{2}-\d{2}$/.test(formData.startDate)) errors.startDate = "Format de date invalide (AAAA-MM-JJ)";
    if (!formData.endDate) errors.endDate = "Veuillez entrer une date de fin";
    else if (!/^\d{4}-\d{2}-\d{2}$/.test(formData.endDate)) errors.endDate = "Format de date invalide (AAAA-MM-JJ)";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const selectedVehicle = vehiclesToDisplay.find((vehicle) => vehicle.id === formData.vehicleId);
    if (!selectedVehicle) {
    ...
    try {
      ...
      setSnackbarMessage(error.message || "Erreur lors de la création du proforma");
      setSnackbarSeverity("error");
    }
    setOpenSnackbar(true);
    setSubmitting(false);
    ...
    return (
      <Box>
        ...
        <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
          ...
          <Typography variant="body2">Client: {selectedClient ? selectedClient.lastName : "Non sélectionné"}</Typography>
        </Box>
          ...
          <Table size="small">
            ...
            <TableBody>
              <TableRow>
                ...
                <TableCell>{selectedVehicle ? `${selectedVehicle.marque} ${selectedVehicle.modele}` : "Non sélectionné"}</TableCell>
              ...
              <TableRow>
                <TableCell colSpan={8} align="right"><strong>TOTAL</strong></TableCell>
                <TableCell><strong>{totalPrice.toLocaleString()} Ar</strong></TableCell>
              </TableRow>
              <TableRow>
                <TableCell colSpan={9}>
                  <Typography variant="body2">
                    Montant total avec carburant: <strong>{totalPrice.toLocaleString()} Ar</strong>
                  </Typography>
                  <Typography variant="body2">
                    Arrêté la présente facture proforma à la somme de : <strong>QUATRE CENT MILLE ARIARY</strong>
                  </Typography>
                </TableCell>
              </TableRow>
            </TableBody>
  ...
  const downloadPreviewAsPDF = () => {
    ...
    doc.text(`Client: ${selectedClient ? selectedClient.lastName : "Non sélectionné"}`, 150, 20);
    doc.setFontSize(12);
    doc.setFont("helvetica", "bold");
    doc.text(`FACTURE PROFORMA N° ${generateInvoiceNumber()}`, 105, 40, { align: "center" });
    doc.setFontSize(10);
    doc.text(formData.contractReference || "Non spécifiée", 105, 50, { align: "center" });

    const tableData = [
      [
        "1",
        selectedVehicle ? `${selectedVehicle.marque} ${selectedVehicle.modele}` : "Non sélectionné",
    ...
    (doc as any).autoTable({
      startY: 60,
      head: [["Réf.", "Moto", "Numéro", "Destination", "Date", "Jour", "Carburant", "Prix unitaire", "Prix total"]],
      body: tableData,
      theme: "grid",
      styles: { fontSize: 8 },
    ...
    doc.setFontSize(10);
    doc.text(`Montant total avec carburant: ${totalPrice.toLocaleString()} Ar`, 20, finalY + 10);
    doc.text("Arrêté la présente facture proforma à la somme de : QUATRE CENT MILLE ARIARY", 20, finalY + 20);
    doc.text(`${formData.regionName || "Antananarivo"}, le ${currentDate}`, 20, finalY + 40);
  ...
  return (
    <ThemeProvider theme={customTheme}>
      <Box sx={{ backgroundColor: '#f9fafb', minHeight: '100vh', p: isMobile ? 2 : 4 }}>
        {loading && <Typography sx={{ mb: 2 }}>Chargement des données...</Typography>}
        {error && !loading && <Typography color="error" sx={{ mb: 2 }}>{error}</Typography>}
        {!loading && !error && clients.length === 0 && (
          <Typography color="warning" sx={{ mb: 2 }}>Aucun client disponible.</Typography>
        )}
        {!loading && !error && allVehicles.length === 0 && (
          <Typography color="warning" sx={{ mb: 2 }}>Aucun véhicule disponible.</Typography>
        )}

        <Grid container spacing={3} mb={isMobile ? 2 : 4}>
          <Grid item xs={12}>
            <Typography variant="h4">Créer un Proforma</Typography>
            <Typography variant="body1">Remplissez les informations ci-dessous pour créer un nouveau proforma.</Typography>
          </Grid>
        </Grid>
        ...
        <Grid container spacing={3}>
            ...
            <DashboardCard sx={{ p: isMobile ? 2 : 3 }}>
              <form onSubmit={handleSubmit}>
                <Grid container spacing={isMobile ? 2 : 3}>
                  <Grid item xs={12} sm={6}>
                    <FormControl fullWidth error={!!formErrors.clientId}>
                      ...
                      <Select
                        ...
                        label="Client"
                        sx={{ borderRadius: '8px', '& .MuiSelect-select': { padding: '10px 14px' } }}
                      >
                        {clients.map((client) => (
                          <MenuItem key={client.id} value={client.id}>{client.lastName}</MenuItem>
                        ))}
                      </Select>
                      {formErrors.clientId && <Typography color="error" variant="body2">{formErrors.clientId}</Typography>}
                    </FormControl>
                  </Grid>
                      ...
                      >
                        {vehiclesToDisplay.length === 0 && (
                          <MenuItem disabled value="">
                            {formData.startDate && formData.endDate ? "Aucun véhicule disponible pour ces dates" : "Aucun véhicule disponible"}
                          </MenuItem>
                        )}
                        {vehiclesToDisplay.map((vehicle) => (
                          <MenuItem key={vehicle.id} value={vehicle.id}>{vehicle.nom} ({vehicle.marque} {vehicle.modele})</MenuItem>
                        ))}
                      </Select>
                      {formErrors.vehicleId && <Typography color="error" variant="body2">{formErrors.vehicleId}</Typography>}
                    </FormControl>
                  </Grid>
                  ...
                  <Grid item xs={12} sm={6}>
                    <FormControl fullWidth error={!!formErrors.status}>
                      ...
                      {formErrors.status && <Typography color="error" variant="body2">{formErrors.status}</Typography>}
                    </FormControl>
                  </Grid>
                </Grid>
                ...
                <Box sx={{ mt: 3, display: 'flex', justifyContent: 'flex-end', gap: 2 }}>
                  <CancelButton
                    variant="outlined"
                    onClick={() => setFormData({ clientId: "", vehicleId: "", startDate: "", endDate: "", regionName: "", contractReference: "", notes: "", status: "" })}
                  >
                    Annuler
                  </CancelButton>
                  <PrimaryButton type="submit" variant="contained" disabled={submitting}>
              ...
              {pdfUrl && (
                <Box sx={{ mt: 2 }}>
                  <Link href={pdfUrl} download="proforma.pdf" target="_blank" rel="noopener noreferrer">
          ...
          <Grid item xs={12} md={6}>
            <PreviewCard elevation={3}>
              {generatePreview()}
              <Box sx={{ mt: 2 }}>
                <PrimaryButton variant="contained" onClick={downloadPreviewAsPDF}>
