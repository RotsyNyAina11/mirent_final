...
const VehiculeList = () => {
  ...
  const { vehicles, loading, vehiclesError } = useSelector((state: RootState) => state.vehicles);
  ...
  const filteredVehicles = vehicles.filter((v) => {
    const matchLieu = selectedLieu ? prixParLieu[v.id]?.[selectedLieu] !== undefined : true;
    const matchPlaces = selectedPlaces ? v.nombrePlace.toString() === selectedPlaces : true;
  ...
  const allTypes = Array.from(new Set(vehicles.map(v => v.type?.type).filter(Boolean)));
            ...
            >
              {vehiculeActuel && (
                <>
                  ...
                  <Typography>
                    <strong>Immatriculation :</strong> {vehiculeActuel.immatriculation}
                  </Typography>
                  <Typography>
                    <strong>Nombre de places :</strong> {vehiculeActuel.nombrePlace}
                  </Typography>
                  <Typography>
