...
const VehiclesList: React.FC = () => {
  ...
  const filteredVehicles = vehicles.filter((veh: any) =>
  ...
  return (
    <Box p={3}>
      {/* Titre */}
      <Typography variant="h4" fontWeight="bold" textAlign="center" mb={3} color="primary.main">
          ...
          {currentVehicles.map((veh: any) => (
            <Grid item xs={12} sm={6} md={4} key={veh.id}>
              <Card sx={{ boxShadow: 3, borderRadius: 2, transition: "0.3s", "&:hover": { transform: "scale(1.05)" } }}>
                <CardContent>
                  <Typography variant="h6" fontWeight="bold">{veh.nom}</Typography>
                  ...
                  <Typography variant="body2" sx={{ color: veh.status === "disponible" ? "green" : "red" }}>
                    Statut: {veh.status}
                  </Typography>
                </CardContent>
      ...
      >
        <Box sx={{ padding: 3, bgcolor: "white", borderRadius: 2, width: "300px", margin: "auto", marginTop: "20%" }}>
          <Typography variant="h6" gutterBottom>Êtes-vous sûr de vouloir supprimer ce véhicule ?</Typography>
          <Box sx={{ display: "flex", justifyContent: "space-between" }}>
            <Button variant="outlined" onClick={() => setOpenDeleteModal(false)} color="secondary">
              Annuler
            </Button>
            <Button
              variant="contained"
