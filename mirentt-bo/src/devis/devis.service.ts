...
export class DevisService {
  ...
  private async processDevisItem(item: CreateDevisItemDto | UpdateDevisItemDto): Promise<any> {
    if (!item.regionId) {
      throw new BadRequestException(`Chaque item doit spécifier un 'regionId'.`);
    }
    if (!item.vehiculeId) {
      throw new BadRequestException(`Chaque item doit spécifier un 'vehiculeId'.`);
    }
    ...
    if (!region) {
      throw new BadRequestException(`Région avec l'ID ${item.regionId} introuvable.`);
    }
    if (!region.prix) {
      throw new BadRequestException(`Aucun prix défini pour la région avec l'ID ${item.regionId} (${region.nom_region}).`);
    }
    ...
    if (!vehicule) {
      throw new BadRequestException(`Véhicule avec l'ID ${item.vehiculeId} introuvable.`);
    }
  ...
  async create(createDevisDto: CreateDevisDto): Promise<Devis> {
    ...
    const numberOfDays = this.calculateNumberOfDays(parsedStartDate, parsedEndDate);
    if (numberOfDays <= 0) {
    ...
    if (!client) {
      throw new BadRequestException(`Client avec l'ID ${clientId} introuvable.`);
    }
    ...
    for (const item of items) {
      if (selectedVehicleIds.has(item.vehiculeId)) {
        throw new BadRequestException(`Le véhicule avec l'ID ${item.vehiculeId} est sélectionné plusieurs fois dans ce devis.`);
      }
  ...
  async update(id: string, updateDevisDto: UpdateDevisDto): Promise<Devis> {
    ...
    if (updateDevisDto.clientId && updateDevisDto.clientId !== devis.clientId) {
      const newClient = await this.clientService.findOne(updateDevisDto.clientId);
      if (!newClient) {
        throw new BadRequestException(`Client with ID ${updateDevisDto.clientId} not found.`);
      }
    ...
    if (updateDevisDto.startDate) {
      ...
      if (isNaN(parsed.getTime())) throw new BadRequestException('Start date is invalid.');
      newStartDate = parsed;
    }
    if (updateDevisDto.endDate) {
      const parsed = new Date(updateDevisDto.endDate);
      if (isNaN(parsed.getTime())) throw new BadRequestException('End date is invalid.');
      newEndDate = parsed;
    }
    ...
    if (numberOfDays <= 0) {
      throw new BadRequestException('Number of rental days must be greater than zero after update.');
    }
    ...
    if (updateDevisDto.includesFuel !== undefined) {
        ...
        if (devis.includesFuel) {
            if (updateDevisDto.fuelCostPerDay === undefined || updateDevisDto.fuelCostPerDay <= 0) {
                throw new BadRequestException('If fuel is included, "fuelCostPerDay" must be provided and be a positive number.');
            }
            devis.fuelCostPerDay = updateDevisDto.fuelCostPerDay;
        } else {
            devis.fuelCostPerDay = undefined; 
            if (updateDevisDto.fuelCostPerDay !== undefined && updateDevisDto.fuelCostPerDay > 0) {
                throw new BadRequestException('If fuel is not included, "fuelCostPerDay" should not be provided or should be zero.');
            }
        }
    } else if (devis.includesFuel && updateDevisDto.fuelCostPerDay !== undefined && updateDevisDto.fuelCostPerDay <= 0) {
        // If includesFuel was true and fuelCostPerDay is updated to <= 0, it's an error
        throw new BadRequestException('If fuel is currently included, "fuelCostPerDay" must remain a positive number or includesFuel must be set to false.');
    } else if (devis.includesFuel && updateDevisDto.fuelCostPerDay !== undefined) {
    ...
    if (updateDevisDto.items) {
      ...
      for (const item of updateDevisDto.items) {
         if (item.vehiculeId && selectedVehicleIds.has(item.vehiculeId)) {
            throw new BadRequestException(`Le véhicule avec l'ID ${item.vehiculeId} est sélectionné plusieurs fois dans les items de mise à jour.`);
         }
         if (item.vehiculeId) {
            selectedVehicleIds.add(item.vehiculeId);
         }
        ...
        newTotalAmount += processedItem.unitPrice * processedItem.quantity * numberOfDays;
      }
